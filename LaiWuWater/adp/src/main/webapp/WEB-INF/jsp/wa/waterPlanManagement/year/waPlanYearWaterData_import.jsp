    ...
    <form   method="post" enctype="multipart/form-data"  id="monthWaterDataExcel" class="hgform">
        <table width="98%" border="0" cellpadding="0" align="center" cellspacing="1" class="hgtable">
            <tr>
                ...
                <td align="left" ><input name="excelFile" type="file"  class="form-input"  id="planWaterDataFile" /></td>
                <td><input name="import" type="button" value="导&nbsp;&nbsp;入" onclick="userAjaxFileUpload()"/>&nbsp;
                    <a href="${ctx}/static/template/年计划用水情况表.xls" target="_blank">模板下载</a>
                </td>
            </tr>
            <tr>
...
<script type="text/javascript">
    ...
    function userAjaxFileUpload(){
        ...
        $.ajaxFileUpload({
            //处理文件上传操作的服务器端地址
                url:"${ctx}/wa/WaPlanYearWaterData/importExcel",
            fileElementId:'planWaterDataFile',           //文件选择框的id属性
            dataType:'json',                       //服务器返回的格式,可以是json或xml等
            success:function(data, status){
                $.messager.progress("close");
                if (data.result == "success") {
                    $.messager.ok("上传数据成功!",function(){
                        $("#waYearWaterDataWin").window("close");
