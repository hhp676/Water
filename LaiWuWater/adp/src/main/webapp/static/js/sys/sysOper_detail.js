function sysOperDetail(operId) {
	...
	$('#sysOper_form').validate({
		rules:{			
			operCode: { required: true,rangelength:[1,32]},
			operName: { required: true,rangelength:[1,32]},
			operEnname: {stringCheck:true, rangelength:[0,100]},
			orderId: {required: true,rangelength:[1,4],digits:true},
			operIcon: { rangelength:[0,100]},
	...
	function _saveSysOper(isAdd) {
		...
		Hg.getJson(submitUrl,$("#sysOper_form").serializeArray(),function(data){
			if (!data.success) {
				...
				Hg.showErrorMsg("sysOper_form",data.data);
