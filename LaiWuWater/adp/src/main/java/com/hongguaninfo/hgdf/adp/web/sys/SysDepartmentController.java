...
import com.hongguaninfo.hgdf.adp.service.sys.SysDepartmentService;
...
public class SysDepartmentController {
    ...
    @RequestMapping(value = "/showSysDepartmentUserCountDetail/{departId}")
    public String showSysDepartmentUserCountDetail(@PathVariable Integer departId,
            HttpServletRequest request, HttpServletResponse response,
            Model model){
    	model.addAttribute("departId", departId);
    	 OperateTemplete templete = new HttpTemplete(request) {
             protected void doSomething() throws BaseException {
                 str = "sys/sysDepartment_userDetail";
             }
         };
         return templete.operateModel();
    }
    @RequestMapping("/getUserDetailByDepartId")
    @ResponseBody
    @RequiresPermissions(Auths.SYS_DEPARTMENT)
    public Map getUserDetailByDepartId(final SysDepartment dept,final SysUser vo,final BasePage pageRequest, HttpServletResponse response,
            HttpServletRequest request){
    	 OperateTemplete templete = new HttpTemplete(request) {
             protected void doSomething() throws BizException {
            	 sysDepartmentService.getDeptUserDetailList(pageRequest, vo, dept, map);
             }
         };
    ...
    @ResponseBody
    @RequiresPermissions(Auths.SYS_DEPARTMENT)
    public Map addInfoList(final SysDepartment vo,
            final BasePage pageRequest, HttpServletResponse response,
            HttpServletRequest request) {
        OperateTemplete templete = new HttpTemplete(request) {
            protected void doSomething() throws BizException {
