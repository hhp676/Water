...
import com.hongguaninfo.hgdf.adp.dao.sys.SysRoleDao;
...
public class SysRoleService extends BaseService {
    ...
    public List<SysRole> getSysRoleList(BasePage<SysRole> basePage, SysRole vo,
        ...
        for (SysRole bo : list) {
            bo.setIsFinalStr(sysDatadicItemBiz.getItemNameByValue("LOGIC_TAG",
                    bo.getIsFinal() + ""));
            List<SysUser> sysList = (List<SysUser>) sysUserRoleJoinDao.getUserListByRoleId(Integer.parseInt(bo.getRoleId()+""));
            bo.setUserNumber(sysList.size());
        }
        map.put("rows", list);
    ...
    public void deleteById(int id) throws BizException {
        ...
        //处理角色-人员关联信息、角色-权限关联信息
        
        sysUserRoleJoinService.deleteByRoleId(id);
        
        sysRoleAuthJoinService.deleteByRoleId(id);
    }
    ...
    // 角色对应人员列表
    public Map getUserRoleJoinList(BasePage<SysRole> basePage, SysRole vo,
            Map<String, Object> map){
        basePage.setFilters(vo);
        Page<SysRole> page = sysRoleDao.pageQuery("getUserList",basePage);
        List<SysRole> list = page.getResult();
        for (SysRole bo : list) {
//        	 bo.setStatusStr(sysDatadicItemService.getItemNameByValue("USER_STATUS", bo.getStatusStr() + ""));
        	 bo.setDepartName(oaCompanyStructureInfoService.getMainDepartNamesStr(bo.getUserId()));
    	     bo.setAreaName(oaCompanyStructureInfoService.getSecondDepartNamesStr(bo.getUserId()));
        }
        map.put("rows", list);
        map.put("total", page.getTotalCount());
        return map;
